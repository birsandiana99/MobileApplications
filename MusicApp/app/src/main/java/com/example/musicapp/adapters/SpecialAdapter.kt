...
class SpecialAdapter(val context: Context) :
    ...
    private fun loadQueryAll() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                elementsList.add(Song(id = id, name = title, quantity = 0, status = album, price = year.toInt()))
    ...
    override fun onBindViewHolder(holder: SpecialViewAdapter, position: Int) {
        ...
        holder.view.album.text = elementsList[position].status // field3
//        holder.view.genre.text = elementsList[position].genre // field4
        holder.view.year.text = elementsList[position].price.toString() // field5
    }
